...
- [x] Plan 4: Ship — see `docs/superpowers/plans/2026-06-05-upmind-swift-plan-4-ship.md`

## Ship status (2026-06-06)

All four plans complete. The Upmind Swift app is a shippable MVP:

- **42 cognitive games** across 7 constructs, all playable with real generators
- **Auth**: email/password + Apple Sign In via Supabase (anon mode default when Supabase not configured)
- **Local-first persistence**: SwiftData caches every session, SyncWorker pushes to Supabase in background
- **RevenueCat paywall**: weekly $9.99 / yearly $39.99, premium gating on 5 Executive games
- **Onboarding**: 4-step flow (Welcome, Value, Briefing, Paywall) shown once for new users
- **Main app**: Today (daily drill + streak), Games (catalog with search + filter), Leaderboard (mock), Profile (auth + premium)
- **Notifications**: 5 categories, HIG-compliant permission flow
- **PostHog**: 21-event taxonomy, feature flag reader stubbed (PostHog SDK API mismatch)
- **UI**: polished (R3) — glass cards, hero text, gradient CTAs, premium feel
- **Tests**: 86 total, 78 pass + 8 expected-failing snapshot stubs

### Known limitations
- No real Supabase / RevenueCat / PostHog keys in dev. App runs in anonymous mode; paywall purchases don't complete; analytics is a no-op. Drop in real keys via Xcode build settings to activate.
- Leaderboard is mock data (no real Supabase view).
- Survey step in onboarding is a placeholder.
- Snapshot test references not yet recorded (run with `SNAPSHOT_TESTING_RECORD=all`).
- TestFlight build works in Debug; Release archive needs a real signing identity.
- PostHog feature flags are stubbed (SDK API mismatch).
- No remote push notifications (local-only for v1).
